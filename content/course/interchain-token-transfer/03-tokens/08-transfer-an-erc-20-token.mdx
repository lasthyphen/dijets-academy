...
Let's deploy a basic demo ERC20 contract that comes integrated with the CLI on our Dijets ECC.

```bash 
dijets contract deploy erc20
```
and follow this configuration:
```bash 
✔ Local Network
✔ Dijets ECC mydijetschain
...
```bash
dijets key list -l --blockchains mydijetschain --tokens $ERC20_CONTRACT_ECC,native  --keys myAddress,ewoq
```
...
+--------+-------+----------+--------------------------------------------+---------------+-------------------+---------------+
| stored | myAddress | mydijetschain | 0x302Ce9b6346E477ecb774aF73E908a4971B7ce2E | NAT (Native)  |         0 | Local Network |
+        +       +          +--------------------------------------------+---------------+-------------------+---------------+
...
Export the address of the Pre-funded account in the table above to an environment variable:
...
```bash
cast send $ERC20_CONTRACT_ECC --value 0 --private-key $PK "transfer(address,uint256)" $MYADDRESS 200ether --rpc-url mydijetschain
```
...
Again, you can verify the transfer was performed correctly by retrieving the balances of each token in the Dijets ECC.
