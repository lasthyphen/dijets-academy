The Contract Deployer Allowlist allows Dijets ECC to restrict who can deploy smart contracts to the chain. This is useful for preventing unwanted behavior. If an address is not on the allowlist, the chain will reject any contract deployment from that address.
...
This also allows for Dijets ECC to make sure that no other blockchain application just deploys on the L1 and it can be exclusively for a specific use case.