...
Now that the wrapped exchange contract has been deployed, send an ERC20 token to execute a swap for WDJT or DJT from your Dijets ECC to Lothar using the [`cast send`](https://book.getfoundry.sh/reference/cast/cast-send) command in foundry.
