...
description: Get familiar with the DITTO design
updated: 2024-05-31
authors: [ashucoder9]
icon: BookOpen
---

Each token transferrer instance consists of one "home" contract and at least one but possibly many "remote" contracts. Each home contract instance manages one asset to be transferred out to `TokenRemote` instances. The home contract lives on the Dijets ECC where the asset to be transferred exists.

A transfer involves locking the asset as collateral on the home Dijets ECC and minting a representation of the asset on the remote Dijets ECC. The remote contracts, each with a single specified home contract, live on other Dijets ECCs that want to import the asset transferred by their specified home.
...
`TokenRemote` instances are registered with their home contract via a InterECC Messaging message upon creation.
