...
In this example, we will wrap Trader Joe's Factory contract to execute swap operations on the destination chain. Trader Joe's exchange contracts are already deployed on Lothar, which why we are choosing Lothar as the destination chain.
...
import {IWDJT} from "./interface/IWDJT.sol";
...
contract DexERC20Wrapper is Context, IERC20SendAndCallReceiver {
    ...
    function receiveTokens(
        ...
        if (swapOptions.tokenOut == address(0)) {
            ...
            IWDJT(WNATIVE).withdraw(amountOut);
