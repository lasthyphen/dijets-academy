...
? How should the transaction fees be configured on your Blockchain?: 
    ...
    Medium block size / Medium Throughput 15 mil gas per block (Utility Chain's setting)
...
- Allow very computationally expensive operations. The gas limit of the Utility Chain is set to 8,000,000 gwei. Let's pick a number higher than that.
- Target producing one block every 10 seconds
- The same minimum transaction base fee as the Utility Chain (25000000000wei = 25 gwei)
- Only slowly adjust the fee. The base fee change denominator of the Utility Chain is 36.
- Have every block cost at least 500,000 gwei and at most 1,000,000 gwei
- Adjust the block gas cost same as the Utility Chain (value 200000)
...
```
? Do you want the transaction fees to be burned (sent to a blackhole address)? All transaction fees on Dijets are burned by default: 
