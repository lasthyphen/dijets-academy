...
const courseMapping: Record<string, string> = {
  'dijets-fundamentals': 'Dijets Fundamentals',
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    const templateUrl = `${serverUrl}/certificates/Dijets_Academy_Certificate.pdf`;
    ...
    return NextResponse.json(
      { error: 'Failed to generate certificate, contact the Dijets team.', details: (error as Error).message },
      { status: 500 }
    );
  }
}