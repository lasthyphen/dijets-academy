...
- InterECC Messaging Registry Address **(for `mydijetschain2`)**
- InterECC Messaging Manager (our funded address)
- Source Blockchain ID (hexidecimal representation of Dijets Utility Chain)
- Token Home Address (address of `NativeTokenHome.sol` deployed on Dijets Utility Chain in the [Deploy a Home Contract](/course/interchain-token-transfer/06-erc-20-to-erc-20-bridge/03-deploy-home) section)
...
```bash
forge create --rpc-url mydijetschain2 --private-key $PK \
...
```bash
cast send --rpc-url mydijetschain2 --private-key $PK $ERC20_TOKEN_REMOTE_CHAIN2 \
...
### Approve tokens for the Token Remote contract on `mydijetschain`
Now that our `ERC20TokenRemote` contract is deployed and configured on `mydijetschain2`, we need to prepare `mydijetschain` for the token transfer. 
...
"approve(address, uint256)" $ERC20_TOKEN_REMOTE_ECC 2000000000000000000000
```
