...
The primary purpose of these mock contracts is to test cross-chain token transfers and execute contract logic. These contracts implement the `IERC20SendAndCallReceiver` and `INativeSendAndCallReceiver` interfaces to handle token transfers, either for ERC20 tokens or native tokens, across Dijets ECCs.
...
```solidity title="lib/dijets-interchain-token-transfer/contracts/src/mocks/MockERC20SendAndCallReceiver.sol"
