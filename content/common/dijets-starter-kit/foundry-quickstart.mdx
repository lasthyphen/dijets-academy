...
> Do not send any funds on any public blockchain to the testing private key address. The funds will most likely be lost immediately.

When you create an Dijets Subnetwork you will be offered an option to configure the initial token allocation. The second option will allocate 1m tokens to that address so you can easily deploy all contracts and pay for the transactions from that address:

```bash
? How would you like to distribute funds: 
    Airdrop 1 million tokens to a newly generate address (stored key)
  ▸ Airdrop 1 million tokens to the default testing address (do not use in production)
    Customize your airdrop
    Go back to previous step
```

The private key for the ewoq address is already stored in the environment variable $PK in the Dijets Starter Kit. Therefore, we can easily access it by using the following flag for the transactions:

```bash
--private-key $PK
```

This address is obviously not suitable for any production use, since the private key is publicly known. We will look into how we can manage key for the deployment to the Lothar Testnet and the Mainnet later in this course.
...
These aliases are defined in the foundry.toml file in the root of the Dijets Starter Kit.

```toml
[rpc_endpoints]
local-c = "http://localhost:9650/ext/bc/U/rpc"
mydijetschain = "http://localhost:9650/ext/bc/mydijetschain/rpc"
lothar-c = "https://api.dijets-test.network/ext/bc/U/rpc"
dispatch = "https://network.dijets.io/dispatch/testnet/rpc"
```

The RPC URLs defined by default are the following:
- **local-c:** The Utility Chain of your local Dijets network
- **mydijetschain:** An Dijets Subnetwork on your local Dijets network called mydijetschain
- **lothar-c:** The Utility Chain of the Lothar Testnet
- **dispatch:** An Dijets Subnetwork on the Lothar Testnet called Dispatch. It was specifically created for testing Dijets Inter-ECC Messaging dApps. 

The ability to use an RPC with the name of our L1 comes from the alias API in our nodes. You can read more about it [here](https://dijets-docs.vercel.app/guides/node-apis/admin-api#methods).

Feel free to add more RPC URLs to Dijets Subnetworks you create. You can also add more aliases for the RPC URLs to make it easier for you to send transactions to them.
