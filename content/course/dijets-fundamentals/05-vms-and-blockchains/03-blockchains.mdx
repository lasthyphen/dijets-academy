...
When a user wishes to execute an operation on this distributed soda dispenser, they transmit the transaction to the network. The validators then reach consensus on the sequence in which transactions are carried out. In Dijets, validators use Dijets Consensus, which we talked about earlier.
...
![](/course-images/dijets-fundamentals/int.svg)
...
Let’s take the example further. Assume we have 100 validators in our Soda Dispenser Dijets ECC. I submit multiple operations to the validators:
...
This is the fundamental principle of Blockchains in Dijets: A collective of validators operate identical virtual machines, come to consensus on the order of transactions, execute them, and have a uniform view on the machine's current state.
