...
The interfaces `IERC20SendAndCallReceiver` or `INativeSendAndCallReceiver` are used for contracts that handle receiving ERC20 or native tokens of the Interchain Token Transfer protocol. They are similar to the `IJumperReceiver` interface, but they are specifically designed to handle token transfers.
...
```solidity title="lib/dijets-interchain-token-transfer/contracts/src/interfaces/IERC20SendAndCallReceiver.sol"
/**
...
interface IERC20SendAndCallReceiver {
     ...
     * @param originTokenTransferrerAddress Address of the token transferrer that initiated the Jumper message
...
```solidity title="lib/dijets-interchain-token-transfer/contracts/src/interfaces/INativeSendAndCallReceiver.sol"
/**
