...
```bash
forge create --rpc-url mydijetschain --private-key $PK src/my-contracts/TokenReceiver.sol:TokenReceiver --constructor-args $ERC20_CONTRACT_ECC
```
...
```bash
cast send $ERC20_CONTRACT_ECC --private-key $PK "approve(address,uint256)" $ERC20_RECEIVER_ECC 20ether --rpc-url mydijetschain
```
...
```bash
cast send $ERC20_RECEIVER_ECC --private-key $PK "receiveTokens(address,uint256)" $EWOQ 2ether --rpc-url mydijetschain
```
...
```bash
cast call $ERC20_CONTRACT_ECC "balanceOf(address)(uint256)" $ERC20_RECEIVER_ECC --rpc-url mydijetschain
```
