...
Using this analogy, we can say that Dijets goal is to build a grocery store with more checkout lanes and faster scanners that can process more customers (transactions) at a time, and the time it takes for a customer to check out and leave the store representing the transaction's finality in a blockchain.
