...
```bash
forge create --rpc-url mydijetschain --private-key $PK src/my-contracts/PayableExample.sol:PayableExample
```
...
```bash
cast call --rpc-url mydijetschain $PAYABLE_CONTRACT "amountReceived()(uint)"
```
...
```bash
cast send --rpc-url mydijetschain --private-key $PK $PAYABLE_CONTRACT "receiveNative()" --value 100000
```
...
It's important to understand the distinction between Native Tokens and ERC-20 tokens. Native tokens, such as `DJT` or `ETH`, are transferred directly to payable functions within the smart contract. This process is straightforward and involves sending the tokens to the contract's address, invoking the payable function. 
