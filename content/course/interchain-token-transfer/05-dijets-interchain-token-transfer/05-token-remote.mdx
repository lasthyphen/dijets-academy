---
title: Token Remote
description: Learn about the Token Remote, the asset destination component of the DITTO
updated: 2024-05-31
authors: [ashucoder9]
icon: BookOpen
---
...
Once deployed, a `TokenRemote` instance must be registered with its specified `TokenHome` contract. This is done by calling `registerWithHome` on the remote contract, which will send a InterECC Messaging message to the home contract with the information to register.
